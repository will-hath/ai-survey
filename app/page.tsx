...
export default function WelcomePage() {
  ...
  const hostSummary =
    agent?.description ||
    agent?.introMessage ||
    "You'll be speaking with a volunteer supporting our misinformation research.";
  const assignmentReady = Boolean(agent && belief && responderId.trim().length > 0);
  const derivedParticipantName = useMemo(() => {
    const trimmedResponder = responderId.trim();
    if (trimmedResponder) {
      return `Responder ${trimmedResponder}`;
    }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(resolveApiUrl('session'), {
        ...
        headers,
        body: JSON.stringify({
          participantName: derivedParticipantName,
      ...
      <div className="w-full max-w-md rounded-2xl border border-neutral-200 bg-white p-8 shadow-xl">
        <header className="mb-6 space-y-2 text-center">
          ...
          <h1 className="text-2xl font-semibold text-neutral-900">
            Join your conversation with {hostDisplayName}
          </h1>
